...
**Success Criteria** (what must be TRUE):
  ...
  1. Prose `@doc` references to hidden internals are resolved via a `skip_code_autolink_to:` list in `mix.exs docs/0`, and `Rendro.PDF.Font` receives a real internal-marking `@moduledoc` so its typespec autolinks stop warning (verified safe: not in `priv/public_api.json` nor the asserted-hidden contract list).
