...
## Milestone 9: Long-Lived Signatures & Compliance Evidence (Completed)
**Focus:** Turned the proof-backed signing seam into a truthful long-lived-signature story without widening into blanket compliance branding or unproven viewer promises.

* **Artifact-First Long-Lived Augmentation:**
  * `Rendro.Sign.augment/2` adds timestamp and revocation evidence over signed artifacts on a separate seam from `sign/2`, with non-deterministic posture made explicit in artifact metadata.
* **Optional Long-Lived Adapter:**
  * First-party optional pyHanko long-lived path provides timestamp and revocation facts while remaining separate from bare signing and certificate trust ownership.
* **Validator-Backed Posture Classification:**
  * `validate/2` reports cryptographic integrity, timestamp presence, revocation evidence presence, and narrow compliance posture as distinct signals; trust-explicit checks live behind a separate `validate_trust/2` seam.
* **Operationally Enforced Live Proof:**
  * The `long-lived-live-proof` CI lane runs the full `sign → augment → validate` workflow against an offline certomancer-backed PKI/TSA/OCSP fixture, and is required on `main`.
* **Truthful Support Contract:**
  * `priv/support_matrix.json` and `guides/api_stability.md` separate `signing.long_lived` evidence from blanket PDF/A claims, signer trust, viewer behavior, and multi-signature workflows.
* **Archive:**
  * See `.planning/milestones/v2.2-ROADMAP.md` for shipped phase detail.
