...
**Success Criteria** (what must be TRUE):
  ...
  1. Prose `@doc` references to hidden internals are resolved via a `skip_code_autolink_to:` list in `mix.exs docs/0`, and the `Rendro.PDF.Font` typespec autolinks stop warning. **(Deviation, user-approved 2026-06-13):** the original plan to give `Font` an *internal-marking* `@moduledoc` while keeping it out of `priv/public_api.json` proved impossible.
