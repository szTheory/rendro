...
| 126-02-01 | 02 | 1 | POLISH-04 | T-126-05 | SHA-256 evidence is produced by the existing cryptographic primitive and deterministic renderer. | byte golden | `mix test test/rendro/theme/preset_accent_golden_test.exs` | ❌ W0 | ⬜ pending |
...
| 126-03-01 | 03 | 2 | POLISH-01, POLISH-02, POLISH-03 | T-126-09, T-126-10 | Exact tested commit is pushed, dispatched through the pinned advisory job, and only six manifest-bound hashes/PNGs are imported. | advisory CI + artifact contract | `mix test test/guardrails/required_checks_contract_test.exs && test -f tmp/phase126_ci_raster_artifact/manifest.txt` | ❌ dispatch/artifact workflow; ✅ pinned harness | ⬜ pending |
| 126-03-02 | 03 | 2 | POLISH-01, POLISH-02, POLISH-03 | T-126-09, T-126-10 | Human confirms external run SHA, pin digest/version, advisory-only blessing, and exact artifact payload before visual review. | automated artifact fence + manual provenance checkpoint | `test -f tmp/phase126_ci_raster_artifact/manifest.txt && mix test test/guardrails/required_checks_contract_test.exs` plus the Plan 03 exact six-file diff gate and checkpoint approval | ❌ external artifact | ⬜ pending |
| 126-04-01 | 04 | 3 | POLISH-01, POLISH-02, POLISH-03 | T-126-12 | Six stable-row PNGs are reviewed individually full-size for semantic ink, Ticket hierarchy/reference fit, money integrity, and regressions. | automated presence gate + manual visual checkpoint | `test -f tmp/rendro_phase126_review/swiss_invoice_light_page_1.png && test -f tmp/rendro_phase126_review/corporate_classic_invoice_dark_page_1.png && test -f tmp/rendro_phase126_review/editorial_ticket_dark_page_1.png && test -f tmp/rendro_phase126_review/minimal_mono_ticket_dark_page_1.png && test -f tmp/rendro_phase126_review/humanist_payslip_dark_page_1.png && test -f tmp/rendro_phase126_review/brutalist_payslip_dark_page_1.png` plus checkpoint approval | ❌ CI artifact / human disposition | ⬜ pending |
...
|----------|-------------|------------|-------------------|
| The downloaded blessing artifact came from the exact tested commit and repository-pinned PDFium binary, with blessing confined to the advisory job. | POLISH-01, POLISH-02, POLISH-03 | GitHub Actions is an external trust boundary; automated manifest/diff checks are supplemented by an explicit run-log provenance disposition. | Open the Task 126-03-01 run URL, compare its head SHA and pinned-install log with the downloaded manifest and `priv/pdfium_pin.json`, confirm the required test job did not bless, then approve or name the mismatch. |
