...
| 126-03-01 | 03 | 2 | POLISH-01, POLISH-02, POLISH-03 | T-126-09, T-126-10 | Guardrails prove CI preserves ordinary triggers and required/advisory isolation, adds only the dedicated `gsd/phase-126-raster-bless-*` push filter, keeps the adapter compare harness separate, and enables preset blessing/staging only for the exact `push` plus `github.ref_name` prefix guard. The exact implementation commit is pushed as the unique ref tip, the run is selected by workflow/event/branch/exact `headSha`, and its manifest-bound artifact is validated before the six-file import. | advisory CI + isolated-push artifact contract | `mix test test/guardrails/required_checks_contract_test.exs && test -f tmp/phase126_ci_raster_artifact/manifest.txt` plus Plan 03's `gh run list --workflow ci.yml --branch "$CI_REF" --event push` exact-`headSha` selection and exact six-file/six-PNG gates | ❌ push-filter/artifact workflow guardrail; ✅ both raster harnesses | ⬜ pending |
