...
| 126-03-01 | 03 | 2 | POLISH-01, POLISH-02, POLISH-03 | T-126-09, T-126-10 | Guardrails prove the advisory job preserves the adapter compare harness, invokes the preset producer with dispatch-controlled blessing/review output, stages exact payload sources with commit/run/pin provenance, then imports only the artifact from the exact tested commit. | advisory CI + artifact contract | `mix test test/guardrails/required_checks_contract_test.exs && test -f tmp/phase126_ci_raster_artifact/manifest.txt` plus the Plan 03 exact six-file and six-PNG gates | ❌ dispatch/artifact workflow guardrail; ✅ both raster harnesses | ⬜ pending |
