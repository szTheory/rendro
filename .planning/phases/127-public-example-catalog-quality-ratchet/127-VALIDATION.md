...
| 127-03-01 | 03 | 3 | CATALOG-01 | T-127-03, T-127-05 | Pinned isolated generation, exact Invoice/Statement `:line_items_60_plus` constructors with independent `page_count > 1` assertions, four named first/final proof files, bounded review payload, CI separation | guardrail + advisory raster | `mix test test/guardrails/required_checks_contract_test.exs --max-failures 1 && review_dir="$(mktemp -d)" && RENDRO_CATALOG_REVIEW_DIR="$review_dir" mix test test/rendro/catalog_raster_review_test.exs --include raster_snapshot --max-failures 1 && test -f "$review_dir/invoice_line_items_60_plus_page_first.png" && test -f "$review_dir/invoice_line_items_60_plus_page_final.png" && test -f "$review_dir/statement_line_items_60_plus_page_first.png" && test -f "$review_dir/statement_line_items_60_plus_page_final.png"` | ❌ W0 — Plan 03 Task 1 | ⬜ pending |
...
| 127-05-02 | 05 | 5 | CATALOG-01, CATALOG-02, CATALOG-03, CATALOG-04 | T-127-01, T-127-02, T-127-03, T-127-04, T-127-05 | Full source/security/package/CI closure including all-cell exact/null D-10 `preview_copy` audit and mutation coverage | full deterministic + advisory | `mix test`; `mix ci.fast`; `jq -e 'all(.cells[]; if .page_count > 1 then .preview_copy == ("Preview: page 1 of \(.page_count)") elif .page_count == 1 then .preview_copy == null else false end)' assets/rendro/catalog.json`; `mix rendro.catalog.check --pdfium PATH` | ❌ dependent closure | ⬜ pending |
