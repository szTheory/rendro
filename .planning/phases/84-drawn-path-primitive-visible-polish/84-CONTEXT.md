...
- **D-06:** `[informational]` The Brand Book hex palette (`#101827`, `#2C6BED`, `line-300 #D8D2C3`, …) is **web/brand chrome, not an engine color-input format** — it does NOT argue for hex in the API. (Brand colors are still expressible as `{r,g,b}` tuples on any surface.) This is a scoping clarification with no implementation artifact — it is satisfied negatively by D-01 (tuples-only on every surface); no plan implements hex input.
