...
| `scripts/phoenix_clean_room_proof.exs` | Phoenix newcomer evidence maintainer | Run the bounded clean-room proof: `mix run scripts/phoenix_clean_room_proof.exs -- --root PATH --output PATH`. | Sealed capsule prerequisite and temporary project -> redacted advisory proof JSON. | advisory evidence | `.github/workflows/release.yml`, `test/scripts/phoenix_clean_room_proof_test.exs`, and public-release verifier tests. | Remove when the Phoenix newcomer proof contract is retired. |
...
| `scripts/verify_docs.exs` | documentation contract maintainer | Verify documentation claims: `mix run scripts/verify_docs.exs`. See the [Catalog Evidence runbook](../.github/workflows/CATALOG-EVIDENCE.md) for its exact-SHA evidence operator lane. | Public docs and support matrix -> deterministic docs-contract diagnostics. | regression | `mix docs.contract`, `mix ci.fast`, and docs-contract tests. | Remove only with replacement of the documentation contract. |
