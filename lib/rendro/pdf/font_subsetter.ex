defmodule Rendro.PDF.FontSubsetter do
  ...
  def subset(bytes, used_glyphs) when is_binary(bytes) and is_list(used_glyphs) do
    with {:ok, version, num_tables, directory} <- parse_offset_table(bytes),
      ...
      {new_glyf, new_loca} =
        rebuild_glyf_and_loca(required_glyphs, new_num_glyphs, offsets, glyf, index_to_loc_format)

      new_maxp = rebuild_maxp(maxp, new_num_glyphs)

      {new_hhea, new_hmtx} =
        rebuild_hmtx_and_hhea(hhea, hmtx, new_num_glyphs, number_of_h_metrics)
  ...
  defp do_parse_table_directory(
    ...
    table =
      if offset + length <= byte_size(bytes),
        do: binary_part(bytes, offset, length),
        else: :invalid
  ...
  defp parse_index_to_loc_format(<<_::binary-size(50), format::signed-16, _::binary>>),
    do: {:ok, format}
  ...
  defp extract_composites(
         <<num_contours::signed-16, _xmin::signed-16, _ymin::signed-16, _xmax::signed-16,
           _ymax::signed-16, rest::binary>>
       )
  ...
  defp assemble_font(version, tables) do
    ...
    header =
      <<version::binary, num_tables::16, search_range::16, entry_selector::16, range_shift::16>>
  ...
  defp inject_head_checksum(bytes, adjustment) do
    ...
    if offset_of_head_record do
      <<_::binary-size(offset_of_head_record), "head", _checksum::32, head_offset::32, _len::32,
        _::binary>> = bytes

      <<prefix::binary-size(head_offset), head_prefix::binary-size(8), _old_csum::32,
        suffix::binary>> = bytes
