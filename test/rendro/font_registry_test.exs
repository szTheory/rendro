defmodule Rendro.FontRegistryTest do
  ...
  describe "fallback chains" do
    test "register_embedded/4 and register/3 accept fallbacks: [:other_logical_name]" do
      ...
      registry = FontRegistry.register(registry, :primary_builtin, built_in: :helvetica, fallbacks: [:fallback_builtin])
      assert {:ok, %{fallbacks: [:fallback_builtin]}} = FontRegistry.fetch(registry, :primary_builtin)

      # Using a mock binary for embedded
      mock_binary = <<0, 1, 2, 3>>
      registry = FontRegistry.register_embedded(registry, :primary_embedded, {:binary, mock_binary}, fallbacks: [:fallback_embedded])
      assert {:ok, %{fallbacks: [:fallback_embedded]}} = FontRegistry.fetch(registry, :primary_embedded)
    end
    ...
    test "preflight/1 verifies that all fallback logical names actually exist in the registry" do
      ...
      assert {:error, {:missing_fallback_target, :missing_fallback, _path}} = FontRegistry.preflight(registry)
